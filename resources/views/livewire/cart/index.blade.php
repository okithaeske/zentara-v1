        ...
        @else
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2">
                            ...
                            <div class="flex flex-col sm:flex-row gap-4 p-4 border rounded-lg">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-start justify-between gap-4">
                                        ...
                                        <button wire:click="remove({{ $item['id'] }})" class="text-red-600 hover:underline">Remove</button>
                                    </div>

                                    <div class="mt-3">
                                        <div class="inline-flex items-center gap-2">
                                            <label class="text-sm text-gray-600">Qty</label>
                                            <input type="number" min="0" max="999" value="{{ $item['qty'] }}" class="w-20 px-3 py-2 border rounded-lg" wire:change="updateQty({{ $item['id'] }}, $event.target.value)">
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <button wire:click="clear" class="mt-6 text-sm text-gray-600 hover:text-red-600">Clear cart</button>
                </div>
